...
export default function LeadRadarDashboard() {
  ...
  const loadHealth = async () => {
    ...
    catch (e:any) { setHealth({ error: String(e) }); }
  ...
  // Export form
  const [formats, setFormats] = useState<string[]>(["csv", "md"]);
  ...
  const kpi = useMemo(() => {
    ...
    const withEmail = leads.filter((l:any)=>Array.isArray(l.emails_found||l.emails) ? (l.emails_found||l.emails).length>0 : !!l.email).length;
    return {
      totalLeads,
              ...
              <CardContent>
                <div className="rounded-md border overflow-auto">
                  <Table>
                    ...
                    <TableBody>
                      {(jobs?.length? jobs: [{id:"-", type:"-", status:"-"}]).slice(0,50).map((j:any)=> (
                        <TableRow key={j.id}>
                ...
                <CardContent>
                  <div className="rounded-md border overflow-auto">
                    <Table>
                      ...
                      <TableBody>
                        {(leads?.length? leads: [{id:"-", company:"-"}]).map((r:any, i:number)=> (
                          <TableRow key={r.id ?? i}>
